...
echo -e "${CYAN}${NC} Run install script to install vuexy system  ${YELLOW}${NC}"

docker exec -it vuexy-app bash -c "composer install"

echo -e "${CYAN}${NC} Generating app key  ${YELLOW}${NC}"
docker exec -it vuexy-app bash -c "php artisan key:generate"

echo -e "${CYAN}${NC} Migrating Database  ${YELLOW}${NC}"
docker exec -it vuexy-app bash -c "php artisan migrate"
docker exec -it vuexy-app bash -c "php artisan passport:install"
docker exec -it vuexy-app bash -c "php artisan db:seed"
